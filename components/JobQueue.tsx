...
interface JobQueueProps {
  queueType: 'Machining' | 'CAM'
  title: string
  addLink: string
  emptyMessage: string
}

export default function JobQueue({ queueType, title, addLink, emptyMessage }: JobQueueProps) {
  ...
  const renderPriorityBadge = (job: Job) => {
    if (['designer', 'admin'].includes(userRole)) {
        return (
            <select
                className={`px-2 py-1 rounded-full text-xs font-semibold border appearance-none cursor-pointer text-center ${getPriorityColor(job.priority)}`}
                value={job.priority}
                onChange={(e) => handlePriorityChange(job.id, e.target.value)}
                style={{ textAlignLast: 'center' }}
            >
                <option value="Urgent" className="bg-white text-black">Urgent</option>
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6 md:space-y-8">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-bold text-funky-yellow">{title}</h1>
      ...
      <section className="space-y-4">
        <Card className="bg-white text-black min-h-[400px] p-0 overflow-hidden">
           <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
              ...
              <Link href={addLink}>
                  <Button variant="primary" size="sm">Add {queueType} Job</Button>
              </Link>
           </div>
           ...
           <div className="md:hidden p-4 space-y-4 bg-gray-50">
                {jobs.map((job, index) => (
                    <div key={job.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 space-y-3">
                        <div className="flex justify-between items-start">
                            <h3 className="font-bold text-lg">{job.part_name}</h3>
                            {renderPriorityBadge(job)}
                        </div>
                        
                        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                            <div>
                                <span className="block text-xs font-semibold text-gray-400 uppercase">Requester</span>
                                {job.requester}
                            </div>
                            <div>
                                <span className="block text-xs font-semibold text-gray-400 uppercase">Est. Time</span>
                                {job.est_time || '-'}
                            </div>
                        </div>
